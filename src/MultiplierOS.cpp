...
Unit(id, name){
    ...
    this->input_ports = stonne_cfg.m_MSwitchCfg.input_ports;  // 1
    this->output_ports = stonne_cfg.m_MSwitchCfg.output_ports;  // 1
...
// 模拟一个周期的行为
// 1. 接收数据
// 2. 如果left_fifo和top_fifo中都有数据，则弹出一个激活值和一个权重，执行乘法操作，将部分和推入累加缓冲区，并将激活值和权重进行转发
// 3. 调用send()方法发送数据到下、左和缓冲区
