...
private:
    // 每个fifo对象表示一个队列，用于缓存从不同方向接收或发送的数据包
    // accbuffer_fifo用于部分和的缓存 
    ...
    int row_num;
    int col_num;
    int num; //General num, just used for information (num = row_num*ms_cols + col_num) 唯一标识符，通过位置计算得出
...
public:
    ...
    void send(); //Send right, bottom and psum fifos 往右、往下、往部分和发送
